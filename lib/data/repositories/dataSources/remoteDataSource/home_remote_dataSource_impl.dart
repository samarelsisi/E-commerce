...
import 'package:e_commerece_online_c13/data/models/AddToCartResponseDM.dart';
...
class HomeRemoteDataSourceImpl implements HomeRemoteDataSource{
  ...
  @override
  Future<Either<Failures, AddToCartResponseDm>> addToCart(String productId) async{
    ...
    try{
      final List<ConnectivityResult> connectivityResult=await Connectivity().checkConnectivity();
      if(connectivityResult.contains(ConnectivityResult.wifi)||connectivityResult.contains(ConnectivityResult.mobile)){
        var addToCart=AddToCartResponseDm.fromJson(response.data);//convert response to object
        if(response.statusCode!>=200&&response.statusCode!<300){
          return Right(addToCart);
        }
        else{
          return Left(ServerError(errorMsg: addToCart.message!));
        }
      }
      else{
        // todo no internet connection
